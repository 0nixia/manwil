...
class ServiceInvoices(ServiceSiat):
	...
	def send_customer_email(self, invoice):
		...
		att_id_pdf = self.env['ir.attachment'].create(attachment_data_pdf)
		ctx['attachment_ids'] = [(4, att_id.id), (4, att_id_pdf.id)]
	...
	def void_by_cuf(self, cuf: str, reason_cancellation: int):
		...
		if res['codigoEstado'] != 905:
			...
			raise UserError(_('No se pudo anular la factura: %s') % res.get('mensajesList', 'Error desconocido'))
		...
		invoice.write({
			...
			'void_reason': reason_cancellation
		})
