...
class SiatTiposFactura(models.Model):
    _name = 'siat.invoice_type'
    _description = 'Modelo para almacenar la lista de Tipos de Factura(sync)'
    ...
    def sync_model(self, service=None):
        """Get the list of Tipos de Factura from SIAT (once per company)."""
        if service is None:
        ...
        labels = service.sync_invoice_type(sucursal, pos)
        self.__sync_model(labels['listaCodigos'], pos)

    def __sync_model(self, data, pos):
        """Synchronize Tipos de Factura from SIAT on the database."""
        new_records = []
        company_id = self.env.company.id
        existing_codes = set(
            self.env['siat.invoice_type']
        ...
        if new_records:
            self.env['siat.invoice_type'].create(new_records)
            _logger.info('New Tipos de Factura created: %s', new_records)