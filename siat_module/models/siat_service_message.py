...
class SiatServiceMessage(models.Model):
    _name = 'siat.service_message'
    _description = 'Modelo para almacenar la lista de Mensajes Servicio(sync)'
    ...
    def sync_model(self, service=None):
        """Get the list of Mensajes Servicio from SIAT (once per company)."""
        if service is None:
        ...
        labels = service.sync_service_message(sucursal, pos)
        self.__sync_model(labels['listaCodigos'], pos)

    def __sync_model(self, data, pos):
        """Synchronize Mensajes Servicio from SIAT on the database."""
        new_records = []
        company_id = self.env.company.id
        existing_codes = set(
            self.env['siat.service_message']
        ...
        if new_records:
            self.env['siat.service_message'].create(new_records)
            _logger.info('New Mensajes Servicio created: %s', new_records)