...
class SiatTypeDocumentSector(models.Model):
    _name = 'siat.type_document_sector'
    _description = 'Modelo para almacenar la lista de Tipos de documento de sector(sync)'
    ...
    def sync_model(self, service=None):
        """Get the list of Tipos de documento de sector from SIAT (once per company)."""
        if service is None:
        ...
        labels = service.sync_type_documents_sector(sucursal, pos)
        self.__sync_model(labels['listaCodigos'], pos)

    def __sync_model(self, data, pos):
        """Synchronize Tipos de documento de sector from SIAT on the database."""
        new_records = []
        company_id = self.env.company.id
        existing_codes = set(
            self.env['siat.type_document_sector']
        ...
        if new_records:
            self.env['siat.type_document_sector'].create(new_records)
            _logger.info('New Tipos de documento de sector created: %s', new_records)