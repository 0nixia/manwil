...
class SiatAccountMove(models.Model):
    ...
    def action_post_siat(self):
        ...
        for record in self:
            ...
            attachment_ids = []
            try:
                from ..services.service_invoices import ServiceInvoices as _SI
                _svc = _SI(record.with_company(record.company_id).env)
                _xml_bytes = _svc.build_xml_for_invoice(invoice)
                if _xml_bytes:
                    import base64 as _b64
                    _att = record.env['ir.attachment'].create({
                        'name':      'FACTURA_{}.xml'.format(invoice.invoice_number),
                        'datas':     _b64.b64encode(_xml_bytes),
                ...
                _log.getLogger(__name__).exception(
                    'action_post_siat | No se pudo adjuntar XML al chatter move=%d', record.id
                )
            record.message_post(
                body=_("Factura N° %s generada (Sucursal %s / PV %s) y registrada en el SIAT.")
                % (invoice.invoice_number, sucursal_code, pos_code),
