...
class SiatSignificantEvents(models.Model):
    _name = 'siat.significant_event'
    _description = 'Modelo para almacenar la lista de Eventos Significativos(sync)'
    ...
    def sync_model(self, service=None):
        """Get the list of Eventos Significativos from SIAT (once per company)."""
        if service is None:
        ...
        labels = service.sync_significant_events(sucursal, pos)
        self.__sync_model(labels['listaCodigos'], pos)

    def __sync_model(self, data, pos):
        """Synchronize Eventos Significativos from SIAT on the database."""
        new_records = []
        company_id = self.env.company.id
        existing_codes = set(
            self.env['siat.significant_event']
        ...
        if new_records:
            self.env['siat.significant_event'].create(new_records)
            _logger.info('New Eventos Significativos created: %s', new_records)