...
class SiatEmissionTypes(models.Model):
    _name = 'siat.emission_type'
    _description = 'Modelo para almacenar la lista de Tipos de Emision(sync)'
    ...
    def sync_model(self, service=None):
        """Get the list of Tipos de Emision from SIAT (once per company)."""
        if service is None:
        ...
        labels = service.sync_emission_type(sucursal, pos)
        self.__sync_model(labels['listaCodigos'], pos)

    def __sync_model(self, data, pos):
        """Synchronize Tipos de Emision from SIAT on the database."""
        new_records = []
        company_id = self.env.company.id
        existing_codes = set(
            self.env['siat.emission_type']
        ...
        if new_records:
            self.env['siat.emission_type'].create(new_records)
            _logger.info('New Tipos de Emision created: %s', new_records)