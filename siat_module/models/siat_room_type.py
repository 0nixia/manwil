...
class SiatRoomTypes(models.Model):
    _name = 'siat.room_type'
    _description = 'Modelo para almacenar la lista de Tipos Habitacion(sync)'
    ...
    def sync_model(self, service=None):
        """Get the list of Tipos Habitacion from SIAT (once per company)."""
        if service is None:
        ...
        labels = service.sync_room_type(sucursal, pos)
        self.__sync_model(labels['listaCodigos'], pos)

    def __sync_model(self, data, pos):
        """Synchronize Tipos Habitacion from SIAT on the database."""
        new_records = []
        company_id = self.env.company.id
        existing_codes = set(
            self.env['siat.room_type']
        ...
        if new_records:
            self.env['siat.room_type'].create(new_records)
            _logger.info('New Tipos Habitacion created: %s', new_records)