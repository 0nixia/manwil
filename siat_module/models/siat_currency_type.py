...
class SiatTiposDeMoneda(models.Model):
    _name = 'siat.currency_type'
    _description = 'Modelo para almacenar la lista de Tipos de Moneda(sync)'
    ...
    def sync_model(self, service=None):
        """Get the list of Tipos de Moneda from SIAT (once per company)."""
        if service is None:
        ...
        labels = service.sync_currency_type(sucursal, pos)
        self.__sync_model(labels['listaCodigos'], pos)

    def __sync_model(self, data, pos):
        """Synchronize Tipos de Moneda from SIAT on the database."""
        new_records = []
        company_id = self.env.company.id
        existing_codes = set(
            self.env['siat.currency_type']
        ...
        if new_records:
            self.env['siat.currency_type'].create(new_records)
            _logger.info('New Tipos de Moneda created: %s', new_records)